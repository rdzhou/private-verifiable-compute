...
usage() {
    cat <<'EOF'
Usage: ./deploy-gke.sh --namespace=<namespace> [--dry-run] [--verbose] [--help]
EOF
}
...
env_values_file="$script_dir/../envs/gke.yaml"
platform="gke"
namespace=""
tag="latest"
dry_run=false
...
if [ -z "$namespace" ]; then
    echo "Error: the namespace parameter is required, run the script again like ./deploy-gke.sh --namespace=<namespace>" >&2
    exit 1
fi
